  ...
  methods: {
    ...
    getReport () {
      ...
      .then((response) => {
        ...
        Array.prototype.forEach.call(response.body.products.data, function (line) {
          const ProductLine = {
        ...
        for (var i = 0; i < response.body.products.top.labels.length; i++) {
          datasets.push({
            label: response.body.products.top.labels[i],
            backgroundColor: backgroundColor[i],
            totalValue: response.body.products.top.total_value[i].toString(),
            data: [response.body.products.top.data[i]]
          })
        }
